...
from app.utils.markdown_utils import MarkdownConverter
...
def save_figure_to_local(pil_crop, save_dir, image_name, reading_order):
    ...
    try:
        ...
        logger.info(f"Error saving figure: {str(e)}")
...
def convert_pdf_to_images(pdf_path, target_size=896):
    ...
    try:
        ...
        logger.info(f"Error converting PDF to images: {str(e)}")
        return []
...
def save_combined_pdf_results(all_page_results, pdf_path, save_dir):
    ...
    json_path = os.path.join(save_dir, "markdown", json_filename)
...
def map_to_original_coordinates(x1, y1, x2, y2, dims: ImageDimensions) -> Tuple[int, int, int, int]:
    ...
    try:
        ...
        logger.info(f"map_to_original_coordinates error: {str(e)}")
...
def map_to_relevant_coordinates(abs_coords, dims: ImageDimensions):
    ...
    try:
        ...
        logger.info(f"map_to_relevant_coordinates error: {str(e)}")
...
def process_coordinates(coords, padded_image, dims: ImageDimensions, previous_box=None):
    ...
    try:
        ...
        logger.info(f"process_coordinates error: {str(e)}")
        # Return safe values
...
def prepare_image(image) -> Tuple[np.ndarray, ImageDimensions]:
    ...
    try:
        ...
        logger.info(f"prepare_image error: {str(e)}")
...
def crop_margin(img: Image.Image) -> Image.Image:
    ...
    try:
        ...
        if width == 0 or height == 0:
            logger.info("Warning: Image has zero width or height")
            return img
        ...
        logger.info(f"crop_margin error: {str(e)}")
